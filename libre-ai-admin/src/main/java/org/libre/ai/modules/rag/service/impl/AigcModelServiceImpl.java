...
import org.libre.ai.modules.rag.mapper.AigcModelMapper;
...
public class AigcModelServiceImpl extends ServiceImpl<AigcModelMapper, AigcModel> implements AigcModelService {
	...
	public List<AigcModel> getChatModels() {
		List<AigcModel> list = baseMapper
			.selectList(Wrappers.<AigcModel>lambdaQuery().eq(AigcModel::getType, ModelTypeEnum.CHAT.name()));
	...
	public List<AigcModel> getImageModels() {
		List<AigcModel> list = baseMapper
			.selectList(Wrappers.<AigcModel>lambdaQuery().eq(AigcModel::getType, ModelTypeEnum.TEXT_IMAGE.name()));
		list.forEach(this::hide);
		return list;
	}
	...
	public List<AigcModel> getEmbeddingModels() {
		List<AigcModel> list = baseMapper
			.selectList(Wrappers.<AigcModel>lambdaQuery().eq(AigcModel::getType, ModelTypeEnum.EMBEDDING.name()));
		list.forEach(this::hide);
		return list;
	}
	...
	public Page<AigcModel> page(AigcModel data, QueryPage queryPage) {
		...
		Page<AigcModel> iPage = this.page(page,
				Wrappers.<AigcModel>lambdaQuery().eq(AigcModel::getProvider, data.getProvider()));
