-- Dashboard生成器内置应用初始化脚本
-- 用于将Dashboard功能集成到AIGC模块

-- 1. 插入Dashboard专用模型配置（使用环境变量中的配置）
INSERT INTO aigc_model (
    id, 
    type, 
    model, 
    provider, 
    ...
    'dashboard-model-001',
    'CHAT',
    'qwen3', -- 默认模型，可根据实际情况修改
    'OPENAI',
    'Dashboard生成模型',
    12800,
    1.0,
    0.0,
    '${OPENAI_API_KEY}', -- 使用环境变量
    '${OPENAI_BASE_URL}'  -- 使用环境变量
...
- 绝对禁止使用markdown语法包裹代码块',
    '智能生成可视化大屏Dashboard页面，支持多种布局、主题和组件配置',
    NOW(),
    NOW()
) ON CONFLICT (id) DO UPDATE SET
...
-- 3. 为Dashboard应用创建API密钥（可选）
INSERT INTO aigc_app_api (
    id,
    app_id,
    channel,
    api_key,
